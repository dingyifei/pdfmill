...
class TestGetSumatraCachePath:
    ...
    def test_returns_path_with_exe(self):
        with patch("pdfmill.printing.sumatra.get_cache_dir") as mock_cache:
...
class TestGetSumatraDownloadUrl:
    ...
    def test_x64_url(self):
        with patch("pdfmill.printing.sumatra.get_architecture", return_value="x64"):
            url = get_sumatra_download_url()
            assert SUMATRA_VERSION in url
            assert "64" in url

    def test_arm64_url(self):
        with patch("pdfmill.printing.sumatra.get_architecture", return_value="arm64"):
            url = get_sumatra_download_url()
            assert "arm64" in url

    def test_x86_url(self):
        with patch("pdfmill.printing.sumatra.get_architecture", return_value="x86"):
            url = get_sumatra_download_url()
...
class TestDownloadSumatra:
    ...
    def test_already_exists_no_force(self, temp_dir):
        ...
        with patch("sys.platform", "win32"):
            with patch("pdfmill.printing.sumatra.get_sumatra_cache_path", return_value=existing):
...
class TestRemoveSumatra:
    ...
    def test_removes_existing_file(self, temp_dir):
        ...
        with patch("pdfmill.printing.sumatra.get_sumatra_cache_path", return_value=existing):
            with patch("pdfmill.printing.sumatra.get_cache_dir", return_value=temp_dir):
    ...
    def test_not_found_returns_false(self, temp_dir):
        ...
        with patch("pdfmill.printing.sumatra.get_sumatra_cache_path", return_value=nonexistent):
            result = remove_sumatra()
...
class TestGetSumatraStatus:
    ...
    def test_installed(self, temp_dir):
        ...
        with patch("pdfmill.printing.sumatra.SumatraBackend.find_sumatra_pdf", return_value=exe_path):
    ...
    def test_not_installed(self):
        with patch("pdfmill.printing.sumatra.SumatraBackend.find_sumatra_pdf", return_value=None):
            status = get_sumatra_status()
...
class TestFindSumatraPdf:
    ...
    def test_env_var_path_not_exists(self, temp_dir):
        ...
        with patch.dict("os.environ", {"PDFPIPE_SUMATRA_PATH": str(nonexistent)}):
            with patch("pdfmill.printing.sumatra.get_sumatra_cache_path") as mock_cache:
    ...
    def test_cache_path(self, temp_dir):
        ...
        with patch.dict("os.environ", {"PDFPIPE_SUMATRA_PATH": ""}):
            with patch("pdfmill.printing.sumatra.get_sumatra_cache_path", return_value=cache_path):
...
class TestPrintPdf:
    ...
    def test_dry_run_no_subprocess(self, temp_pdf, mock_subprocess_run, capsys):
        with patch("pdfmill.printing.sumatra.SumatraBackend.find_sumatra_pdf") as mock_find:
