"""SumatraPDF printer backend for Windows."""
...
def download_sumatra(force: bool = False) -> Path:
    ...
    Raises:
        PrinterError: If download fails or not on Windows
    """
    if sys.platform != "win32":
...
def get_sumatra_status() -> dict:
    ...
    """
    backend = SumatraBackend()
    path = backend.find_sumatra_pdf(auto_download=False)
    return {
...
class SumatraBackend(PrinterBackend):
    """Printer backend using SumatraPDF on Windows."""

    name = "sumatra"

    @classmethod
    def is_available(cls) -> bool:
        """Check if SumatraPDF backend is available (Windows only)."""
        return sys.platform == "win32"

    def list_printers(self) -> list[str]:
        """List available printers using win32print.

        Returns:
            List of printer names

        Raises:
            PrinterError: If win32print is not available
        """
        try:
            import win32print
    ...
    def find_sumatra_pdf(self, auto_download: bool = True) -> Path | None:
    ...
    def print_pdf(
        ...
        if sumatra_path is None:
            sumatra_path = self.find_sumatra_pdf(auto_download=True)
